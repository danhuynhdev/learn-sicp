...
;; 	(cond ((= m 0) x)
;; 		  ((= m 1) y)
;; 		  (else (display "Argument not 0 or 1: CONS" m))))
;;   dispatch)
;; (define (car z) (z 0))
;; (define (cdr z) (z 1))
...
;;; adjoin-set now grow as O(1)
;;; intersection-set is the same because we still have to use element-of-set?
;;; union-set now grow as O(n)

;;; these thing may be useful if we don't have to care about memory and just
;;; care about if a thing is in the list or not
...
;;; For all three trees the result is the same (1 3 5 7 9 11)

;;; 2.63b
;;; the first procedure is slower because it use append which grow as O(n)
;;; so tree->list-1 grow as O(nlogn) whereas tree->list-2 grow as O(n)
...
;;; partial-tree takes in a list and then calculate the number of elements in
;;; each side like this:
;;; (1 2 3 4 5 6) n=5 -> 5 - (1 entry point) -> 4 -> 4 / 2 -> 2
;;; then make a recursive call to make a tree with that size that will return:
;;; ((tree 1 2) . (3 4 5 6)
;;; The first element of the remaining list is the entry point. Now we just
;;; have to make a tree with the remaining list except the entry point with
;;; size n - (left-size + 1 entry point):
;;; ((tree 4 5) (6))
;;; Now that we have left-side entry-point right-side we can make a tree node
;;; and return (cons tree-node remain)
...
(define (encode-symbol symbol tree)
  ...
  (cond ((leaf? tree) '())
		((in? symbol (left-branch tree))
		 (cons 0 (encode-symbol symbol (left-branch tree))))
		((in? symbol (right-branch tree))
...
;;; the encoding required 84 bits.
;;; If we used a fixed-length code we will need (* 3 36) -> 108 bits
...
;;; We just need one bit for the most frequent symbol, for the least frequent
;;; symbol we will need n-1 bits.
...
;;; 2.73a. It is because they are implement independant
